...
def generate_launch_description():
    ...
    os.environ['GZ_SIM_RESOURCE_PATH'] += ':' + os.path.dirname(get_package_share_directory('learn_mtc'))

    os.environ['IGN_GAZEBO_RESOURCE_PATH'] = os.path.join(get_package_share_directory('learn_mtc'), 'worlds')
    os.environ['IGN_GAZEBO_MODEL_PATH'] = os.path.dirname(get_package_share_directory('franka_description'))
    ...
    spawn = Node(
        ...
        namespace=namespace,
        arguments=[
            '-topic', '/robot_description', '-name', 'panda', '-x', '0.2', '-y', '0', '-z', '1.025', '-allow_renaming', 'true'
        ],
        output='screen',
    )
